...
module.exports = (db) => {
  ...
  router.post("/:currencyId/like", async (req, res) => {
    ...
    try {
      ...
      const coinExists = await db.query(
        `SELECT * FROM liked_cryptos WHERE crypto_url_id = $1 AND user_id = $2;`,
        [currencyId, validUser.rows[0].id]
      ); //checking url from the db
      if (coinExists.rows[0]) {
        return res.status(400).send("The crypto is already in your liked!");
      }
      await db.query(
    ...
    VALUES ($1, $2) RETURNING *;`,
      ...
      return res.status(200).send("Data is inserted into database.");
    } catch (error) {
  ...
  router.get("/mylikes", async (req, res) => {
    ...
    try {
      ...
      const likedCryptos = await db.query(
        `SELECT * FROM liked_cryptos WHERE user_id = $1;`,
        [validUser.rows[0].id]
      );
  ...
  router.post("/:currencyId/unlike", async (req, res) => {
    ...
    try {
      ...
      const { currencyId } = req.params;
      const coinObject = await db.query(
        `SELECT * FROM liked_cryptos WHERE crypto_url_id = $1 AND user_id = $2;`,
        [currencyId, validUser.rows[0].id]
      ); //checking id from the db
      ...
      const updateCryptos = await db.query(
        `DELETE FROM liked_cryptos WHERE crypto_url_id = $1;`,
        [currencyId]
      );
